jQuery(document).ready(function($) {
  ...
  if (Modernizr.history) {
    ...
    $(document).on('click','a',  function(e) {
      ...
      // Check if the link is on the same domain/host
      // If not, handle proper click action
      var sameHost = Boolean($(this)[0].host === window.location.host);

      // Check if the link redirects to wp-admin
      //  If not, handle proper click action
      var ifAdmin = Boolean(url.indexOf('wp-admin') < 0);

      // Check if the link opens in a new tab/window
      // If yes, handle proper click action
      var newWindow = Boolean($(this).attr('target') !== '_blank');

      // Only Ajax load if the links are internal
      if (sameHost && ifAdmin && newWindow) {
